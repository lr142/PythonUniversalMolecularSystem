...
class PDBFile(MolecularFile):
    ...
    def Read(self,molecularSystem,filename):
        ...
        try:
            for i,line in enumerate(contents,start=1):
                ...
                if line.startswith("ATOM") or line.startswith("HETATM"):
                    ...
                    # A fail prevention here. If no element is written in this segment, use the first character in
                    # atom.name as the element. Note that this will cause some problems for elements with 2 chars like
                    # Cl or Pt.
                    if len(atom.element) == 0:
                        if not atom.name[0].isdigit():
        ...
        for atom in tempAtomCollection:
            ...
            # chainID+resSeq shall be used as a unique identifier for residues
            # if there is no chain info in the file, chainID here shall be ' '. This won't affect the program
            # the identifier shall be (temporarily) written to 'serial' of each molecule.
            # Remember that molecules' serials might be changed or reordered later.
            # Refer directly to the chainID, resSeq, and resName segments, which are permanent.
            identifier = '{}{}'.format(chainID,resSeq)
            if identifier in tempMoleculesCollection:
                tempMoleculesCollection[identifier].atoms.append(atom)
            else:
                newMolecule = Molecule()
                newMolecule.serial = identifier
                newMolecule.name = resName
                # Use the resName as its name. It also has a resName property. Molecule.name is mutable, but
                # Molecule.resName is constant
                newMolecule.resName = resName
                newMolecule.chainID = chainID
                newMolecule.resSeq = resSeq
                tempMoleculesCollection[identifier] = newMolecule
